...
private:
  ...
  void writeSourceFileView(StringRef SourceFile, CoverageMapping *Coverage,
                           ...
                           std::vector<StringRef> ObjectFilenames, bool ShowArchExecutables);
...
void CodeCoverageTool::writeSourceFileView(
    ...
    bool ShowFilenames, std::vector<StringRef> ObjectFilenames, bool ShowArchExecutables) {
  ...
  View->setShowArchExecutables(ShowArchExecutables);
  ...
  if (!ViewOpts.hasOutputDirectory() || S.ThreadsRequested == 1) {
    for (const std::string &SourceFile : SourceFiles)
      writeSourceFileView(SourceFile, Coverage.get(), Printer.get(),
                          ShowFilenames, ObjectFilenames, ViewOpts.ShowArchExecutables);
  } else {
    ...
    for (const std::string &SourceFile : SourceFiles)
      Pool.async(
          static_cast<void (CodeCoverageTool:: *)(
              ...
              std::vector<StringRef>, bool)>(&CodeCoverageTool::writeSourceFileView),
          this, SourceFile, Coverage.get(), Printer.get(), ShowFilenames,
          ObjectFilenames, ViewOpts.ShowArchExecutables);
    Pool.wait();
  }
