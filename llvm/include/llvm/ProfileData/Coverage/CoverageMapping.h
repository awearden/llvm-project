...
struct FunctionRecord {
  ...
  void pushRegion(CounterMappingRegion Region, uint64_t Count,
                  uint64_t FalseCount) {
    if (Region.isBranch()) {
      CountedBranchRegions.emplace_back(
          Region, Count, FalseCount); // If either counter is hard-coded to
                                      // zero, then this region represents a
    ...
    CountedRegions.at(CountedRegions.size() - 1).ObjectFilename =
        ObjectFilename;
  }
};
